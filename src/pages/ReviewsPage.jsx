  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const endpoint =
          user?.role === 'provider'
            ? `/reviews?provider_id=${user?._id}&page=${page}&limit=${pageSize}`
            : `/reviews?customer_id=${user?._id}&page=${page}&limit=${pageSize}`;

        const res = await api.get(endpoint);
        if (res.data.success) {
          const list = res.data.data || [];
          setReviews(list);
          setTotalPages(res.data.totalPages || Math.ceil(list.length / pageSize) || 1);
          setTotalCount(res.data.total || res.data.totalReviews || list.length);
        }
      } catch (err) {
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {reviews.map(rev => (
              <div
                key={rev._id}
                className="bg-[#141720] border border-neutral-800 rounded-2xl p-5 flex flex-col justify-between space-y-4 shadow-md"
              >
                <div>
                  <div className="flex items-center justify-between gap-2 mb-3">
                    <div className="flex items-center gap-2.5">
                      <div className="w-9 h-9 rounded-xl bg-orange-500/20 text-orange-400 flex items-center justify-center font-bold text-xs border border-orange-500/30">
                        {(rev.user?.name || rev.service?.title || 'R')
                          .charAt(0)
                          .toUpperCase()}
                      </div>
                      <div>
          ...
          <Pagination
            ...
            totalItems={totalCount || reviews.length}
            pageSize={pageSize}
